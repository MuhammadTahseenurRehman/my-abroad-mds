...
export const AIShowcase = () => (
  <section className="relative overflow-hidden bg-primary text-primary-foreground dark:bg-gradient-to-br dark:from-[hsl(200_44%_9%)] dark:via-[hsl(199_38%_12%)] dark:to-[hsl(200_42%_10%)] dark:text-foreground">
    <div className="absolute inset-0 -z-0 opacity-30 [background:radial-gradient(800px_400px_at_20%_10%,hsl(var(--primary-glow)/0.6),transparent_60%),radial-gradient(600px_300px_at_90%_90%,hsl(var(--accent)/0.4),transparent_60%)] dark:opacity-[0.18]" />
    <div className="container relative grid gap-12 py-24 lg:grid-cols-2 lg:items-center">
      ...
      >
        ...
        <h2 className="font-display text-4xl font-semibold leading-tight md:text-5xl dark:text-foreground">
          One brief. Three clinics. A plan you can trust.
        </h2>
        <p className="mt-5 max-w-lg text-primary-foreground/80 dark:text-muted-foreground">
        ...
        <ul className="mt-7 space-y-3 text-sm">
          {[
            ...
            <li key={t} className="flex items-start gap-3">
              ...
              <span className="text-primary-foreground/90 dark:text-foreground/90">{t}</span>
            </li>
          ))}
        </ul>
        <div className="mt-8 flex flex-wrap gap-3">
          <Button asChild variant="accent" size="lg">
            <Link to="/app/advisor">Try the advisor</Link>
          </Button>
          <Button
            asChild
            variant="ghostOnDark"
      ...
      <motion.div
        ...
        className="relative"
      >
        <div className="rounded-3xl border border-white/15 bg-white/5 p-6 backdrop-blur-md shadow-elegant dark:border-border/70 dark:bg-card/55">
          <div className="flex items-center justify-between">
            <p className="text-xs uppercase tracking-widest text-primary-foreground/60 dark:text-muted-foreground">Trip brief</p>
            <span className="rounded-full bg-accent/20 px-2 py-0.5 text-xs text-accent">Live</span>
          </div>
          <h3 className="mt-2 font-display text-2xl dark:text-foreground">Lisbon → Marrakech, 9 days</h3>
...
const Card = ({
  ...
  <div className="flex items-start gap-4 rounded-2xl border border-white/10 bg-primary/40 p-4 dark:border-border/60 dark:bg-secondary/50">
    <span className="grid h-10 w-10 flex-none place-items-center rounded-xl bg-accent/15 text-accent">
      <Icon className="h-5 w-5" />
    </span>
    <div className="flex-1">
      <p className="text-xs uppercase tracking-wide text-primary-foreground/60 dark:text-muted-foreground">{title}</p>
      <p className="font-display text-lg dark:text-foreground">{value}</p>
      <p className="text-sm text-primary-foreground/70 dark:text-muted-foreground">{detail}</p>
    </div>
    {cta && (
      <Link
        to="/find-doctors"
        className="self-center rounded-full bg-accent px-3 py-1.5 text-xs font-medium text-accent-foreground transition-transform hover:scale-105"
      >
        Book
      </Link>
    )}
